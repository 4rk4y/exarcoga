    ...
    for next_point in points_iter {
        ...
        while !next_point_found && counter < 100 {
            ...
            tiles.push((
                ...
                tile_y as isize,
                match (side, tile.0) {
                    (current, next) if (current, next) == (0, 0) => 1,
                    (current, next) if (current, next) == (0, 1) => 34,
                    (current, next) if (current, next) == (1, 1) => 18,
                    (current, next) if (current, next) == (1, 2) => 2,
                    (current, next) if (current, next) == (2, 2) => 1,
                    (current, next) if (current, next) == (2, 3) => 0,
                    (current, next) if (current, next) == (3, 3) => 16,
                    (current, next) if (current, next) == (3, 0) => 32,
                    (current, next) if (current, next) == (2, 1) => 4,
                    (current, next) if (current, next) == (1, 0) => 20,
                    (current, next) if (current, next) == (3, 2) => 3,
                    (current, next) if (current, next) == (0, 3) => 19,
                    (current, next) => {
